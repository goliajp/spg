...
|-----|--------------------|-----------|------------|
| **v6.3.0** | `Engine`-level plan cache. New struct `PreparedPlan { stmt: Statement, statistics_version: u64, source_tables: Vec<String> }`. `Engine` grows `prepared_plans: BTreeMap<String, PreparedPlan>` keyed on the SQL string, capped at 256 entries (LRU via `VecDeque<String>`). New API `Engine::prepare_cached(sql) -> &PreparedPlan` — on hit returns the cached entry; on miss runs the existing `prepare()` path then inserts. `Engine::execute_prepared` takes `&Statement` so cache lookup stays zero-copy. | `crates/spg-engine/src/plan_cache.rs` 6 unit tests + `tests/e2e_plan_cache::repeat_prepare_returns_cached_plan` + `…::lru_evicts_oldest_at_cap` + perf gate: `prepare_cached` hit-path latency ≤ 5 % of cold-path latency on a 5-table JOIN | v6.2.8 |
