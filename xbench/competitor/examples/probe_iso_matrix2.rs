...
fn savepoint_x_rebase() {
    ...
    e.execute_in("INSERT INTO t VALUES (100)", IMPLICIT_TX)
        .unwrap();
    println!(
        "sp1 pre-rollback (PG [1,2,100]): {}",
        q(&mut e, "SELECT x FROM t ORDER BY x", tx)
    );
    e.execute_in("ROLLBACK TO SAVEPOINT s", tx).unwrap();
    println!(
        "sp2 post-rollback (PG [1,100]): {}",
        q(&mut e, "SELECT x FROM t ORDER BY x", tx)
    );
    e.execute_in("COMMIT", tx).unwrap();
    println!(
        "sp3 committed (PG [1,100]): {}",
        q(&mut e, "SELECT x FROM t ORDER BY x", IMPLICIT_TX)
    );
}
...
fn delete_then_insert_same_key() {
    ...
    println!(
        "di1 tx view (PG [1,11]): {}",
        q(&mut e, "SELECT id, v FROM t", tx)
    );
    let commit = e.execute_in("COMMIT", tx);
    println!("di2 commit (PG ok): {commit:?}");
    println!(
        "di3 final (PG [1,11]): {}",
        q(&mut e, "SELECT id, v FROM t", IMPLICIT_TX)
    );
}
