...
fn seed(db: &mut spg_embedded::Database, t: &str, ddl: &str, vidx: bool) {
    ...
    if vidx {
        db.execute(&format!("CREATE INDEX {t}_v ON {t} (v)")).unwrap();
    }
    let mut i = 1;
    while i <= N {
...
fn vac_ms(db: &mut spg_embedded::Database, t: &str) -> f64 {
    db.execute(&format!("UPDATE {t} SET g = g + 1 WHERE v BETWEEN 20000 AND 40000")).unwrap();
    let t0 = Instant::now();
...
fn main() {
    ...
    seed(&mut db, "t0", "CREATE TABLE t0 (id INT NOT NULL, g INT NOT NULL, v INT NOT NULL)", false);
    seed(&mut db, "t1", "CREATE TABLE t1 (id INT PRIMARY KEY, g INT NOT NULL, v INT NOT NULL)", false);
    seed(&mut db, "t2", "CREATE TABLE t2 (id INT PRIMARY KEY, g INT NOT NULL, v INT NOT NULL)", true);
    for round in 0..3 {
        let a = vac_ms(&mut db, "t0");
        let b = vac_ms(&mut db, "t1");
        let c = vac_ms(&mut db, "t2");
        println!("round {round}: 0-idx {a:7.3} ms | 1-idx {b:7.3} ms | 2-idx {c:7.3} ms  (per-index ≈ {:.3})", c - b);
    }
}
