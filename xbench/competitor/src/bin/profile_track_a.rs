//! v7.37.5-A2b — samply-targeted micro-bench for Track A /api/conversations.
//!
//! Opens prod snapshot + runs Track A SQL via spg-embedded `Database`.
//! Track A = the GROUP BY + 14-aggregate + 3-correlated-scalar + NOT EXISTS
//! shape that produces /api/conversations rows. Reference SQL in
//! `~/workspace/stables/mailrs/.claude/notes/spg-7.37.3-prod-conversations-real-sql-2026-06-18.md`.
//!
//! Run:
//!     CARGO_PROFILE_RELEASE_DEBUG=true \
//!         cargo build --release -p spg-bench-competitor --bin profile_track_a
//!     samply record --rate 5000 ./target/release/profile_track_a
...
fn main() {
    ...
    // Track A: cold time = first hit (~200 ms in decomp doc); warm = repeat.
    // We measure BOTH separately: 1 cold + 30 warm.
