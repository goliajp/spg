...
//! WINs, so the remaining loss frontier is writes. Two deliberate fairness
//! choices, both tilted AGAINST SPG so a WIN is real and a LOSS is real:
//!
//!  * SPG runs `spg_embedded::Database::open_path` — the real durable customer
//!    path (WAL + fsync semantics), NOT the bare in-memory Engine the read
//!    panel uses.
//!  * PostgreSQL runs with `SET synchronous_commit = off` — its fastest
//!    commit mode — so PG's fsync-per-commit inflation (which buried the
//!    Phase-0 single-row INSERT numbers) can't mask an SPG loss.
//!
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // ---- SPG embedded, durable file backend ----
    ...
    let spg = bench_engine(&mut |sql| {
        db.execute(sql).unwrap();
    });
    drop(db);
    let _ = std::fs::remove_dir_all(&dir);

    // ---- PostgreSQL 18, fastest commit mode ----
    ...
    let pool = AnyPoolOptions::new()
        ...
        .connect(&pg_url)
        .await?;
    sqlx::query("DROP TABLE IF EXISTS wb").execute(&pool).await?;
    sqlx::query("SET synchronous_commit = off")
        .execute(&pool)
        .await?;
    // Bridge the sync closure over the async pool via block-in-place on a
    // handle; simplest: collect SQL through a channel-free direct runner.
    let rt = tokio::runtime::Handle::current();
    let pool2 = pool.clone();
    let pg = tokio::task::block_in_place(move || {
        bench_engine(&mut |sql| {
            rt.block_on(async {
    ...
    println!("# SPG = spg-embedded DURABLE (open_path); PG18 synchronous_commit=off");
    println!("| shape        |    SPG ms |   PG18 ms |  SPG/PG | verdict |");
    println!("|--------------|----------:|----------:|--------:|---------|");
    for (i, (name, _)) in SHAPES.iter().enumerate() {
        let ratio = spg[i] / pg[i];
        let verdict = if ratio <= 1.0 { "WIN" } else if ratio < 1.2 { "~tied" } else { "LOSS" };
        println!(
            "| {:<12} | {:>9.3} | {:>9.3} | {:>6.2}× | {:<7} |",
            name, spg[i], pg[i], ratio, verdict
        );
    }
    Ok(())
}
