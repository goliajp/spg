...
| **Transactions** (v7.39.1) | `BEGIN`/`COMMIT`/`ROLLBACK` with a clone-on-BEGIN shadow catalog. Single-writer locking; own-write visibility inside the TX. **v7.39.0**: and the guarantees a session is TOLD it has are the ones it has. `REPEATABLE READ` freezes the view at BEGIN — a concurrent commit is invisible to it and visible to a `READ COMMITTED` one, matching PostgreSQL 18.6 cell for cell — and `default_transaction_isolation` now reaches it, where before SPG accepted the setting, read it back, and ran read committed anyway. `READ ONLY` is enforced: every statement PG 18.6 refuses is refused with its message and SQLSTATE `25006`, including `CREATE TEMP TABLE`, an `UPDATE` that matches no rows, and `nextval()` inside an ordinary `SELECT`; before this release `BEGIN READ ONLY; INSERT …` answered `INSERT 0 1` and committed. The level is reported live by all four surfaces that answer for it — two of them used to hold constants that contradicted the engine. The MySQL dialect defaults to `REPEATABLE READ`, as MySQL 9.7.2 does. **v7.39.1**: `SET LOCAL` belongs to the connection that wrote it. The undo log was one list for the whole engine while the values it restores are per connection, and the test for "are we in a transaction" was true when ANY connection was — so a connection could come out of its own COMMIT holding a value only another connection had ever set, measured with two live sessions. Applications set a tenant with `set_config('app.tenant_id', …, true)` once per request and read it back; neither half of this is visible without concurrency. |
| **MySQL dialect** (v7.39.1) | A session enters it through `SET sql_mode`, and the wire enters it on connect. **v7.39.0**: a fresh connection is STRICT, which it was not — `VARCHAR(3) <- 'abcdef'` stored `'abc'` and `TINYINT <- 999` stored `127`, silently and reported as success, while `@@sql_mode` told that same connection `STRICT_TRANS_TABLES`. MySQL 9.7.2 answers `ERROR 1406` and `ERROR 1264`. `SET sql_mode = ''` still bends values to fit, as MariaDB does — the default was wrong, not the switch. `sql_mode` is now one list and lists only what SPG enforces, each flag verified by the statement only that flag refuses; `NO_ZERO_DATE` and `NO_ZERO_IN_DATE` were enforced all along and never claimed, and `NO_ENGINE_SUBSTITUTION` is claimed and kept: `ENGINE=NONSUCH` answers `ERROR 1286 (42000) Unknown storage engine 'NONSUCH'`, checked before anything is created. (This sentence said the opposite through v7.39.0, describing the state before the flag was implemented later in that same release — a customer-facing claim that measurement contradicted.) `SHOW VARIABLES` reports what the session SET rather than the compiled-in default — it did the latter for every canonical name, `SET NAMES` included. Identity is one constant per fact (`9.7.2-spg`); `have_ssl` and `tx_isolation`, removed from MySQL in 8.0.26 and 8.0.3, are gone from here too. **v7.39.1**: `"…"` opens a STRING here, as it does in MySQL — SPG read it as an identifier, so `SELECT "abc"` answered `column "abc" does not exist` and ordinary MySQL SQL failed on an error naming a column nobody wrote; `ANSI_QUOTES` turns the identifier rule back on and was not modelled at all before. A value that will not fit is refused the way MySQL refuses it (`1264 (22003)` out of range, `1406 (22001)` too long, with `at row N`), where the code was PostgreSQL's and a driver branching on it branched wrong. `@@collation_database`, `@@character_set_database` and the `SHOW VARIABLES` inventory answer what MySQL 9.7.2 answers under an identical handshake, and `information_schema` carries the charset and collation a reflection reads off every column. **Not** kept: an unquoted identifier is still folded to lower case, and a backticked one is not — the two spellings name different tables. |
