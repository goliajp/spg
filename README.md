...
| **Sorting** (v7.38.20) | An eight-byte key and a row number, not the rows: an integer's whole value with its sign bit flipped, or text's first eight bytes big-endian. On 400,000 rows against PostgreSQL 18, ordering bytes on both sides — integer **0.79x**, short text 1.42x, long distinct text 1.53x (was 6.12x). Declaring a collation costs 1.00x–1.26x, where it cost 4.09x. |
