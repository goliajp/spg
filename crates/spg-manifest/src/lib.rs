...
//! v7.1 — extracted into a standalone crate so `spg-embedded`
//! can read + write the same `SPGMAN01` v10 manifest that
//! `spg-server` does. Cross-binary compatibility was already
//! implied by the STABILITY freeze; this crate makes it
//! mechanically true (one impl, two consumers).
