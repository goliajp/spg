...
async fn main() -> Result<(), EngineError> {
    ...
    for row in db.query("SELECT id, title, done FROM tasks ORDER BY id").await? {
        let Value::BigInt(id) = row[0] else { unreachable!() };
        let Value::Text(ref title) = row[1] else { unreachable!() };
        let Value::Bool(done) = row[2] else { unreachable!() };
