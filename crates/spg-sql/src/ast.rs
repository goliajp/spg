...
pub enum ValidateOnlyKind {
    ...
    /// better and shifted thirteen discriminants, which
    /// `cargo-semver-checks` calls out as its own breaking change
    /// (`enum_no_repr_variant_discriminant_changed`) for anyone casting
    /// the enum. Reading order is not worth a break nobody asked for.
    ///
    /// It used to share `RoleName` with the three role statements, and one question separates
...
pub enum AlterTableTarget {
    ...
    /// IDENTITY [( … )]`. It used to lower to
    /// [`AlterTableTarget::SetColumnAutoIncrement`], which marks the
    /// column auto-increment and loses WHICH identity it is — so the
    /// column read `is_identity NO` afterwards, and PostgreSQL's two
    /// refusals (a NULLABLE column, and a second ADD) never fired.
    ...
    /// which changes the flavour of a column that is already one. It
    /// was swallowed by the SET tail's catch-all.
    AlterColumnSetIdentityKind { column: String, kind: IdentityKind },
...
/// 9.0.0 — which `GENERATED … AS IDENTITY` a column was declared as.
///
/// PostgreSQL distinguishes them everywhere a client can see: an
/// `ALWAYS` column refuses an explicit INSERT value without `OVERRIDING
/// SYSTEM VALUE`, and `information_schema` / `pg_attribute` name the
/// flavour (`ALWAYS` / `BY DEFAULT`, `a` / `d`).
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
...
impl IdentityKind {
    ...
    #[must_use]
    pub const fn attidentity(self) -> &'static str {
        match self {
            Self::Always => "a",
            Self::ByDefault => "d",
        }
...
pub struct ColumnDef {
    ...
    /// v7.38 (read01) — the identity flavour, when the column is one.
    ///
    /// 9.0.0 — was `identity_always: bool`, which answered "is this the
    /// ALWAYS one" and could not tell `BY DEFAULT` from a plain
    /// `serial`: both read `false`. So `information_schema.columns`
    /// reported `is_identity NO` and `pg_attribute.attidentity` an empty
    /// string for a `GENERATED BY DEFAULT AS IDENTITY` column, where
    /// PostgreSQL 18.6 answers `YES` / `BY DEFAULT` / `d`.
    ///
    /// Both flavours also set `auto_increment`; `None` with
    /// `auto_increment` is `serial`.
...
fn fmt_alter_target(f: &mut fmt::Formatter<'_>, t: &AlterTableTarget) -> fmt::Result {
    match t {
        ...
        AlterTableTarget::AlterColumnAddIdentity { column, kind, .. } => write!(
            f,
            "ALTER COLUMN {} ADD GENERATED {} AS IDENTITY",
            quote_ident(column),
            kind.generation()
        ),
        AlterTableTarget::AlterColumnSetIdentityKind { column, kind } => write!(
            f,
            "ALTER COLUMN {} SET GENERATED {}",
            quote_ident(column),
            kind.generation()
        ),
