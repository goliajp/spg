//! v4.18 VACUUM / ANALYZE / CLUSTER / REINDEX no-ops.
...
fn unique_tmpdir() -> PathBuf {
    let nanos = std::time::SystemTime::now()
        ...
        .unwrap()
        .as_nanos();
    let p = crate::common::tmp_base().join(format!("spg-e2e-vac-{nanos}"));
...
#[test]
fn vacuum_and_friends_return_clean_cc() {
    ...
    for (sql, tag) in [
        ...
        ("VACUUM mytable", "VACUUM"),
        ("ANALYZE", "ANALYZE"),
        ("ANALYZE mytable", "ANALYZE"),
        ("CLUSTER", "CLUSTER"),
        ("REINDEX TABLE mytable", "REINDEX"),
    ] {
        send_query(&mut s, sql);
        let msgs = drain_until_ready(&mut s);
        assert_cc_tag(&msgs, tag);
    }
}
