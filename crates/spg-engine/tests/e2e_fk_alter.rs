...
fn add_constraint_on_compatible_existing_data_succeeds() {
    ...
    eng.execute(
        "ALTER TABLE o ADD CONSTRAINT fk_user FOREIGN KEY (uid) REFERENCES u(id)",
    )
    .unwrap();
...
fn drop_constraint_removes_enforcement() {
    ...
    // Drop the constraint.
    eng.execute("ALTER TABLE o DROP CONSTRAINT fk_user").unwrap();
...
fn duplicate_constraint_name_is_rejected() {
    ...
    let r = eng.execute(
        "ALTER TABLE o ADD CONSTRAINT shared FOREIGN KEY (b) REFERENCES u(id)",
    );
