...
fn unsigned_int_rejects_negative_insert() {
    ...
    e.execute("CREATE TABLE t (id INT UNSIGNED NOT NULL)").unwrap();
...
fn unsigned_int_accepts_positive_insert() {
    let mut e = Engine::new();
    e.execute("CREATE TABLE t (id INT UNSIGNED NOT NULL)").unwrap();
    e.execute("INSERT INTO t VALUES (1), (42), (1000000)").unwrap();
}
...
fn unsigned_bigint_rejects_negative() {
    let mut e = Engine::new();
    e.execute("CREATE TABLE t (id BIGINT UNSIGNED NOT NULL)").unwrap();
...
fn unsigned_flag_persists_through_catalog_roundtrip() {
    ...
    let col = table.schema().columns.iter().find(|c| c.name == "id").unwrap();
