...
fn real_precision_and_rendering() {
    ...
    assert_eq!(text(&mut e, "SELECT ('3.14159265358979'::real)::text"), "3.1415927");
    assert_eq!(text(&mut e, "SELECT (0.1::real)::text"), "0.1");
    assert_eq!(text(&mut e, "SELECT (12345678::real)::text"), "1.2345678e+07"); // sci past exp 5
...
fn real_typeof_and_arithmetic() {
    ...
    assert_eq!(text(&mut e, "SELECT pg_typeof(1.5::real + 2.5::real)"), "real");
    assert_eq!(text(&mut e, "SELECT (1.5::real + 2.5::real)::text"), "4");
    assert_eq!(text(&mut e, "SELECT (2.0::real * 3.0::real)::text"), "6");
    assert_eq!(text(&mut e, "SELECT pg_typeof(1.5::real + 1)"), "double precision");
    assert_eq!(text(&mut e, "SELECT pg_typeof(1.5::real + 2.5::float8)"), "double precision");
}
...
fn real_column_roundtrip() {
    let mut e = Engine::new();
    assert_eq!(text(&mut e, "SELECT pg_typeof(x) FROM (SELECT 0.1::real AS x) t"), "real");
    assert_eq!(text(&mut e, "SELECT (x)::text FROM (SELECT 0.1::real AS x) t"), "0.1");
}
