...
fn inet_and_cidr_cast_into_each_other() {
    ...
    for (sql, want) in [
        ...
        ("SELECT '192.168.1.5/24'::inet::cidr::text", "192.168.1.0/24"),
...
fn a_cidr_always_shows_its_mask_length() {
    ...
    assert_eq!(one(&mut e, "SELECT '192.168.1.5'::inet::cidr"), "192.168.1.5/32");
    ...
    assert_eq!(one(&mut e, "SELECT '192.168.1.0/24'::cidr"), "192.168.1.0/24");
    assert_eq!(one(&mut e, "SELECT '192.168.1.5/24'::inet"), "192.168.1.5/24");
}
...
fn input_errors_take_pgs_wordings() {
    let mut e = Engine::new();
    for (sql, want) in [
        ("SELECT '999.1.1.1'::inet", "invalid input syntax for type inet: \"999.1.1.1\""),
        (
            "SELECT '192.168.1.5/33'::inet",
            "invalid input syntax for type inet: \"192.168.1.5/33\"",
        ),
        ("SELECT 'notanip'::inet", "invalid input syntax for type inet: \"notanip\""),
        ("SELECT 'notanip'::cidr", "invalid input syntax for type cidr: \"notanip\""),
    ] {
...
fn the_network_core_is_unchanged() {
    ...
    for (sql, want) in [
        ...
        ("SELECT '192.168.1.0/24'::inet >>= '192.168.1.0/24'::inet", "t"),
        ("SELECT '192.168.1.0/24'::inet && '192.168.1.128/25'::inet", "t"),
        ...
        ("SELECT inet_same_family('192.168.1.5'::inet, '::1'::inet)", "f"),
