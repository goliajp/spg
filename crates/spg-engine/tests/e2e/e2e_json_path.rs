...
fn hash_arrow_missing_key_returns_null() {
    let mut eng = Engine::new();
    let v = one_value(&mut eng, r#"SELECT '{"a":1}' #> '{b}'"#);
...
fn contains_top_level_subset() {
    let mut eng = Engine::new();
    let v = one_value(&mut eng, r#"SELECT '{"a":1,"b":2,"c":3}' @> '{"a":1}'"#);
...
fn contains_disjoint_keys_false() {
    let mut eng = Engine::new();
    let v = one_value(&mut eng, r#"SELECT '{"a":1}' @> '{"b":1}'"#);
...
fn contains_array_subset() {
    let mut eng = Engine::new();
    let v = one_value(&mut eng, r#"SELECT '[1,2,3,4]' @> '[2,4]'"#);
    assert_eq!(v, Value::Bool(true));
}
...
fn null_propagates_through_path_ops() {
    let mut eng = Engine::new();
    let v = one_value(&mut eng, r#"SELECT NULL #> '{a}'"#);
    assert_eq!(v, Value::Null);
}
