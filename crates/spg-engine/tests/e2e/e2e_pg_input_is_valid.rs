...
fn integer_valid_and_invalid() {
    ...
    assert!(!as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('99999999999999', 'integer')"
    )));
    // Fits in bigint.
    assert!(as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('99999999999999', 'bigint')"
    )));
}
...
fn boolean_variants() {
    ...
    for input in &["true", "false", "t", "f", "TRUE", "yes", "no", "1", "0"] {
        ...
        assert!(
            as_bool(&first(&mut e, &sql)),
            "{input} should be valid boolean"
        );
    }
    assert!(!as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('maybe', 'boolean')"
    )));
}
...
fn numeric_and_float() {
    ...
    assert!(as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('3.14', 'numeric')"
    )));
    assert!(as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('1e10', 'double precision')"
    )));
    assert!(!as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('abc', 'numeric')"
    )));
}
...
fn text_always_valid() {
    ...
    assert!(as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('anything', 'text')"
    )));
    assert!(as_bool(&first(
        &mut e,
        "SELECT pg_input_is_valid('', 'text')"
    )));
}
