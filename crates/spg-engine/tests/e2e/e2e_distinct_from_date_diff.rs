...
fn is_distinct_from_uses_equality_semantics() {
    ...
    assert_eq!(
        one(&mut e, "SELECT (1 IS NOT DISTINCT FROM 1.0)::text"),
        "true"
    );
    assert_eq!(
        one(&mut e, "SELECT (1 IS DISTINCT FROM 1.0)::text"),
        "false"
    );
    assert_eq!(one(&mut e, "SELECT (1 IS DISTINCT FROM 2.0)::text"), "true");
    assert_eq!(
        one(&mut e, "SELECT (1.5 IS NOT DISTINCT FROM 1.50)::text"),
        "true"
    );
    assert_eq!(
        one(
            &mut e,
            "SELECT (1::bigint IS NOT DISTINCT FROM 1::int)::text"
        ),
        "true"
    );
    // Same-type inequality and the NULL-safe arms still hold.
    assert_eq!(
        one(&mut e, "SELECT ('a' IS DISTINCT FROM 'a')::text"),
        "false"
    );
    assert_eq!(
        one(&mut e, "SELECT (NULL::int IS DISTINCT FROM 1)::text"),
        "true"
    );
    assert_eq!(
        one(&mut e, "SELECT (NULL IS NOT DISTINCT FROM NULL)::text"),
        "true"
    );
}
...
fn date_minus_date_is_integer() {
    ...
    assert_eq!(
        one(
            &mut e,
            "SELECT (DATE '2020-03-01' - DATE '2020-01-01')::text"
        ),
        "60"
    );
    assert_eq!(
        one(
            &mut e,
            "SELECT pg_typeof(DATE '2020-03-01' - DATE '2020-01-01')::text"
        ),
        "integer"
    );
}
