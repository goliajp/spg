...
fn distinct_over_derived_sources() {
    ...
    assert_eq!(
        nrows(&mut e, "SELECT DISTINCT x FROM (VALUES (1),(1),(2)) v(x)"),
        2
    );
    assert_eq!(
        nrows(
            &mut e,
            "SELECT DISTINCT x FROM (SELECT 1 AS x UNION ALL SELECT 1) t"
        ),
        1
    );
    // unnest.
    assert_eq!(
        nrows(&mut e, "SELECT DISTINCT u FROM unnest(ARRAY[1,1,2,2]) u"),
        2
    );
    ...
    assert_eq!(
        nrows(
            &mut e,
            "SELECT DISTINCT x FROM (VALUES (1.0),(1.00),(1.000)) v(x)"
        ),
        1
    );
    ...
    assert_eq!(
        nrows(
            &mut e,
            "SELECT DISTINCT x FROM (VALUES (1),(1),(2),(3)) v(x) ORDER BY x LIMIT 2"
        ),
        2
    );
