...
fn numeric_special_arithmetic() {
    ...
    assert_eq!(t(&mut e, "SELECT ('Infinity'::numeric + 1)::text"), "Infinity");
    assert_eq!(t(&mut e, "SELECT ('Infinity'::numeric - 'Infinity'::numeric)::text"), "NaN");
    assert_eq!(t(&mut e, "SELECT ('Infinity'::numeric * 0)::text"), "NaN");
    assert_eq!(t(&mut e, "SELECT ('Infinity'::numeric * (-2))::text"), "-Infinity");
    assert_eq!(t(&mut e, "SELECT ('Infinity'::numeric / 'Infinity'::numeric)::text"), "NaN");
    ...
    assert_eq!(t(&mut e, "SELECT (-('Infinity'::numeric))::text"), "-Infinity");
}
...
fn numeric_special_compare_and_order() {
    ...
    assert_eq!(t(&mut e, "SELECT 'NaN'::numeric > 'Infinity'::numeric"), "true");
    ...
    assert_eq!(
        t(&mut e, "SELECT string_agg(x::text, ',' ORDER BY x) FROM (VALUES ('-Infinity'::numeric),('Infinity'::numeric),('NaN'::numeric),(0::numeric),((-5)::numeric)) v(x)"),
        "-Infinity,-5,0,Infinity,NaN"
    );
    assert_eq!(t(&mut e, "SELECT max(x)::text FROM (VALUES ('Infinity'::numeric),(1),('-Infinity'::numeric)) v(x)"), "Infinity");
