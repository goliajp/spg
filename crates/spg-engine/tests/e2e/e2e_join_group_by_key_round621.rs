...
fn seed() -> Engine {
    ...
    e.execute("CREATE TABLE na (id INT PRIMARY KEY, s TEXT, g INT)").unwrap();
    // nb deliberately has NO primary key, so it is the unlicensed side.
    e.execute("CREATE TABLE nb (bid INT, t TEXT, aid INT)").unwrap();
    e.execute("CREATE TABLE uu (id INT UNIQUE, s TEXT)").unwrap();
    e.execute("INSERT INTO na VALUES (1,'x',10),(2,'y',20),(3,'z',10)").unwrap();
    e.execute("INSERT INTO nb VALUES (10,'p',1),(11,'q',1),(12,'r',2)").unwrap();
...
fn round621_a_grouped_key_licenses_its_own_side() {
    let mut e = seed();
    assert_eq!(
        vals(&mut e, "SELECT a.s, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id ORDER BY 1"),
        vec!["x|2", "y|1"]
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s, a.g, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id ORDER BY 1"),
        vec!["x|10|2", "y|20|1"],
        "every other column of the licensed side, not just one"
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s || b.t, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id, b.t ORDER BY 1"),
        vec!["xp|1", "xq|1", "yr|1"],
        "inside an expression, beside a grouped column of the other side"
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s, count(*) FROM na a, nb b WHERE b.aid=a.id GROUP BY a.id ORDER BY 1"),
        vec!["x|2", "y|1"],
        "the comma spelling of the join"
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s, count(*) FROM na a LEFT JOIN nb b ON b.aid=a.id GROUP BY a.id ORDER BY 1"),
        vec!["x|2", "y|1", "z|1"],
        "and an outer join, where the unmatched row still forms its own group"
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id HAVING max(b.t) > '' ORDER BY 1"),
        vec!["x|2", "y|1"],
        "with a HAVING over the other side"
    );
    assert_eq!(
        vals(&mut e, "SELECT a.s, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id ORDER BY a.g"),
        vec!["x|2", "y|1"],
...
fn round621_a_key_does_not_travel_along_the_join_condition() {
    let mut e = seed();
    assert!(
        err(&mut e, "SELECT a.s, count(*) FROM na a JOIN na c ON a.id=c.id GROUP BY c.id")
...
fn round621_the_other_side_stays_refused() {
    let mut e = seed();
    assert!(
        err(&mut e, "SELECT b.t, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id")
            .contains(r#"column "b.t" must appear in the GROUP BY clause"#),
        "nb has no primary key, so nothing determines its columns: {}",
        err(&mut e, "SELECT b.t, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id")
    );
    assert!(
        err(&mut e, "SELECT a.s, b.t, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id")
            .contains(r#"column "b.t" must appear in the GROUP BY clause"#),
        "the licensed column beside the unlicensed one — the unlicensed one decides"
    );
    assert!(
        err(&mut e, "SELECT a.s, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.g")
            .contains(r#"column "a.s" must appear in the GROUP BY clause"#),
        "grouping by a NON-key column of the licensed side licenses nothing"
    );
    assert!(
        err(&mut e, "SELECT a.s, count(*) FROM na a JOIN nb b ON b.aid=a.id GROUP BY a.id ORDER BY b.t")
            .contains(r#"column "b.t" must appear in the GROUP BY clause"#),
        "ORDER BY is walked too"
    );
    assert!(
        err(&mut e, "SELECT u.s, count(*) FROM na a JOIN uu u ON u.id=a.id GROUP BY a.id")
    ...
    assert!(
        err(&mut e, "SELECT a.s, count(*) FROM na a JOIN uu u ON u.id=a.id GROUP BY u.id")
            .contains(r#"column "a.s" must appear in the GROUP BY clause"#),
...
fn round621_single_table_is_unchanged() {
    ...
    assert_eq!(
        vals(&mut e, "SELECT s, count(*) FROM na a GROUP BY a.id ORDER BY 1"),
        vec!["x|1", "y|1", "z|1"]
    );
    assert!(
