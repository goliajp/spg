...
fn build_engine(disable: bool) -> Engine {
    ...
    e.execute("CREATE TABLE u (id INT NOT NULL PRIMARY KEY, name TEXT NOT NULL, org INT NOT NULL)")
        .unwrap();
...
fn joinfold_off_matches_joinfold_on() {
    ...
    let sql = "SELECT u.id, u.name, o.label FROM u JOIN o ON u.org = o.id ORDER BY u.id";
