...
fn row_to_json_builds_object() {
    ...
    assert_eq!(text(&mut e, "SELECT row_to_json(row(1,'a'))"), "{\"f1\":1,\"f2\":\"a\"}");
    assert_eq!(text(&mut e, "SELECT row_to_json(row(1,'a',true))"), "{\"f1\":1,\"f2\":\"a\",\"f3\":true}");
    assert_eq!(text(&mut e, "SELECT to_json(row(1,'a'))"), "{\"f1\":1,\"f2\":\"a\"}");
    // NULL field → JSON null.
    assert_eq!(text(&mut e, "SELECT row_to_json(row(1,NULL,'x'))"), "{\"f1\":1,\"f2\":null,\"f3\":\"x\"}");
    // Nested composite → nested object.
    assert_eq!(text(&mut e, "SELECT row_to_json(row(1, row(2,3)))"), "{\"f1\":1,\"f2\":{\"f1\":2,\"f2\":3}}");
    // to_jsonb canonicalises (spaces after ':' and ',').
    assert_eq!(text(&mut e, "SELECT (to_jsonb(row(5,'q')))::text"), "{\"f1\": 5, \"f2\": \"q\"}");
}
...
fn composite_text_form() {
    ...
    assert_eq!(text(&mut e, "SELECT (row(1,NULL,'x,y'))::text"), "(1,,\"x,y\")");
    assert!(matches!(
