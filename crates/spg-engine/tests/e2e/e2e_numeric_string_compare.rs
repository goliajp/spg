...
fn numeric_compared_to_string_literal_coerces() {
    ...
    assert_eq!(scalar(&mut e, "SELECT (100::bigint > '50')"), Value::Bool(true));
...
fn where_int_column_equals_string_literal() {
    ...
    e.execute("INSERT INTO ct VALUES (5, 'hello'), (7, 'world')").unwrap();
    // The ubiquitous ORM shape: int column = quoted string.
    assert_eq!(scalar(&mut e, "SELECT i FROM ct WHERE i = '5'"), Value::Int(5));
    // Text vs Text is unaffected — still a string comparison.
    assert_eq!(scalar(&mut e, "SELECT (t = 'hello') FROM ct WHERE i = 5"), Value::Bool(true));
}
