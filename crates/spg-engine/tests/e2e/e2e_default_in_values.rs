...
fn default_uses_declared_column_default() {
    ...
    e.execute("CREATE TABLE t(a int, b int DEFAULT 42)")
        .unwrap();
    e.execute("INSERT INTO t VALUES (1, DEFAULT)").unwrap();
    e.execute("INSERT INTO t (a, b) VALUES (2, DEFAULT)")
        .unwrap();
    // A DEFAULT in a permuted column list resolves the right column.
    e.execute("INSERT INTO t (b, a) VALUES (DEFAULT, 3)")
        .unwrap();
    // Mixed DEFAULT / explicit across a multi-row INSERT.
    e.execute("INSERT INTO t VALUES (10, DEFAULT), (11, 5), (12, DEFAULT)")
        .unwrap();
    assert_eq!(
...
fn default_on_serial_takes_next_value() {
    ...
    e.execute("INSERT INTO t (id, v) VALUES (DEFAULT, 'x')")
        .unwrap();
...
fn default_on_generated_column_computes_it() {
    let mut e = Engine::new();
    e.execute("CREATE TABLE t(a int, b int GENERATED ALWAYS AS (a*10) STORED)")
        .unwrap();
    e.execute("INSERT INTO t VALUES (5, DEFAULT)").unwrap();
    e.execute("INSERT INTO t (a, b) VALUES (6, DEFAULT)")
        .unwrap();
    assert_eq!(
        col(&mut e, "SELECT (a||':'||b)::text FROM t ORDER BY a"),
        vec!["5:50", "6:60"]
...
fn default_is_null_for_a_column_without_a_default() {
    ...
    e.execute("INSERT INTO t VALUES (1, DEFAULT)").unwrap();
    assert_eq!(
        col(&mut e, "SELECT COALESCE(b::text,'NULL') FROM t"),
        vec!["NULL"]
    );
