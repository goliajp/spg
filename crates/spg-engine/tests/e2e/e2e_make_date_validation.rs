...
fn make_timestamp_validates_day_of_month() {
    let mut e = Engine::new();
    assert_eq!(
        text(&mut e, "SELECT (make_timestamp(2024,6,15,14,30,45.5))::text"),
        "2024-06-15 14:30:45.5"
    );
    assert_eq!(text(&mut e, "SELECT (make_timestamp(2024,2,29,0,0,0))::text"), "2024-02-29 00:00:00");
    // Out-of-range date / time components error, no roll-over.
    assert!(e.execute("SELECT make_timestamp(2024,2,30,12,0,0)").is_err());
    assert!(e.execute("SELECT make_timestamp(2024,4,31,0,0,0)").is_err());
    assert!(e.execute("SELECT make_timestamp(2024,6,15,25,0,0)").is_err());
}
