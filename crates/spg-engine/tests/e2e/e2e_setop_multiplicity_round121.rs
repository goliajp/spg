...
fn intersect_all_is_min() {
    ...
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES('a'),('a'),('a')) INTERSECT ALL (VALUES('a'),('a'))) s"), 2);
    // Multi-group: 2→min(2,3)=2, 3→min(1,1)=1.
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES(1),(2),(2),(3)) INTERSECT ALL (VALUES(2),(2),(2),(3))) s"), 3);
}

#[test]
fn except_all_is_saturating_subtract() {
    let mut e = Engine::new();
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES('a'),('a'),('a')) EXCEPT ALL (VALUES('a'))) s"), 2);
    // left < right → 0 (no negative counts).
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES('a')) EXCEPT ALL (VALUES('a'),('a'),('a'))) s"), 0);
    // Multi-group: 1→2−1=1, 2→1−0=1.
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES(1),(1),(2)) EXCEPT ALL (VALUES(1),(3))) s"), 2);
}

#[test]
fn distinct_variants_emit_zero_or_one() {
    let mut e = Engine::new();
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES('a'),('a'),('a')) INTERSECT (VALUES('a'),('a'))) s"), 1);
    // Distinct EXCEPT removes the group entirely if the right has any match.
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES('a'),('a')) EXCEPT (VALUES('a'))) s"), 0);
}

#[test]
fn null_groups_with_null() {
    let mut e = Engine::new();
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES(NULL::int)) INTERSECT (VALUES(NULL::int))) s"), 1);
    assert_eq!(count(&mut e, "SELECT count(*) FROM ((VALUES(NULL::int),(1)) EXCEPT ALL (VALUES(1))) s"), 1);
}
