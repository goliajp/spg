...
fn json_build_array_uses_comma_space() {
    ...
    assert_eq!(scalar(&mut e, "SELECT json_build_array(1,'a',true)::text"), "[1, \"a\", true]");
...
fn jsonb_builders_canonicalise() {
    ...
    assert_eq!(scalar(&mut e, "SELECT jsonb_build_object('a',1,'b','x')::text"), "{\"a\": 1, \"b\": \"x\"}");
    assert_eq!(scalar(&mut e, "SELECT jsonb_build_array(1,'a',true)::text"), "[1, \"a\", true]");
}
...
fn json_builders_nest() {
    let mut e = Engine::new();
    assert_eq!(
        scalar(&mut e, "SELECT json_build_object('a', json_build_array(1,2))::text"),
        "{\"a\" : [1, 2]}"
    );
    assert_eq!(
        scalar(&mut e, "SELECT json_build_array(json_build_object('k',1), 2)::text"),
        "[{\"k\" : 1}, 2]"
    );
}
...
fn json_object_from_array_uses_space_colon_space() {
    let mut e = Engine::new();
    assert_eq!(scalar(&mut e, "SELECT json_object('{a,1,b,2}')::text"), "{\"a\" : \"1\", \"b\" : \"2\"}");
    assert_eq!(scalar(&mut e, "SELECT jsonb_object('{a,1,b,2}')::text"), "{\"a\": \"1\", \"b\": \"2\"}");
}
...
fn to_json_and_row_to_json_stay_compact() {
    ...
    assert_eq!(scalar(&mut e, "SELECT to_json(ARRAY[1,2,3])::text"), "[1,2,3]");
    assert_eq!(scalar(&mut e, "SELECT array_to_json(ARRAY[1,2,3])::text"), "[1,2,3]");
    assert_eq!(scalar(&mut e, "SELECT row_to_json(ROW(1,2))::text"), "{\"f1\":1,\"f2\":2}");
}
