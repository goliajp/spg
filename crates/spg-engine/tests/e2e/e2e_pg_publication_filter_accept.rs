...
fn publication_with_column_list_parses() {
    ...
    ddl(
        &mut e,
        "CREATE TABLE orders (id INT, customer INT, total NUMERIC)",
    );
    ddl(
        &mut e,
...
fn publication_with_col_list_and_row_filter_parses() {
    ...
    ddl(
        &mut e,
        "CREATE TABLE orders (id INT, customer INT, status TEXT)",
    );
    ddl(
        &mut e,
...
fn publication_filter_table_visible_in_pg_publication() {
    ...
    let r = e
        .execute("SELECT pubname FROM pg_catalog.pg_publication WHERE pubname = 'pub_e'")
        .unwrap();
