...
fn accepts_non_zero_padded_and_alt_separators() {
    ...
    for lit in ["2020-01-05", "2020-1-5", "2020-01-5", "2020-1-05", "2020/01/05", "2020/1/5", "2020.01.05", " 2020-1-5 "] {
...
fn still_rejects_out_of_range_and_garbage() {
    ...
    for lit in [
        ...
        assert!(e.execute(&format!("SELECT '{lit}'::date")).is_err(), "{lit} should be rejected");
    }
}
