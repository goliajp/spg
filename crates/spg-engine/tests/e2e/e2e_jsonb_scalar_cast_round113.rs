...
fn jsonb_number_to_integer_targets() {
    ...
    assert!(matches!(scalar(&mut e, "SELECT ('5'::jsonb)::int"), spg_storage::Value::Int(5)));
    assert!(matches!(scalar(&mut e, "SELECT ('2.5'::jsonb)::int"), spg_storage::Value::Int(3)));
    assert!(matches!(scalar(&mut e, "SELECT ('-2.5'::jsonb)::int"), spg_storage::Value::Int(-3)));
    assert!(matches!(scalar(&mut e, "SELECT ('1.5'::jsonb)::int"), spg_storage::Value::Int(2)));
    assert!(matches!(scalar(&mut e, "SELECT ('5'::jsonb)::bigint"), spg_storage::Value::BigInt(5)));
    assert!(matches!(scalar(&mut e, "SELECT ('5'::jsonb)::smallint"), spg_storage::Value::SmallInt(5)));
}
...
fn jsonb_bool_and_null() {
    let mut e = Engine::new();
    assert!(matches!(scalar(&mut e, "SELECT ('true'::jsonb)::bool"), spg_storage::Value::Bool(true)));
    assert!(matches!(scalar(&mut e, "SELECT ('false'::jsonb)::bool"), spg_storage::Value::Bool(false)));
    // JSON null → SQL NULL (not the string "null").
    assert!(matches!(scalar(&mut e, "SELECT ('null'::jsonb)::int"), spg_storage::Value::Null));
    assert!(matches!(scalar(&mut e, "SELECT ('null'::jsonb)::bool"), spg_storage::Value::Null));
}
...
fn jsonb_non_castable_kinds_error() {
    ...
    assert!(err(&mut e, "SELECT ('\"7\"'::jsonb)::int").contains("cannot cast jsonb string to type integer"));
    assert!(err(&mut e, "SELECT ('true'::jsonb)::int").contains("cannot cast jsonb boolean to type integer"));
    assert!(err(&mut e, "SELECT ('[1,2]'::jsonb)::int").contains("cannot cast jsonb array to type integer"));
    assert!(err(&mut e, "SELECT ('{\"a\":1}'::jsonb)::int").contains("cannot cast jsonb object to type integer"));
    // A number is not castable to bool, and a string is not castable to numeric.
    assert!(err(&mut e, "SELECT ('5'::jsonb)::bool").contains("cannot cast jsonb numeric to type boolean"));
    assert!(err(&mut e, "SELECT ('\"7\"'::jsonb)::numeric").contains("cannot cast jsonb string to type numeric"));
    // The numeric-target message carries no doubled class prefix.
    assert!(!err(&mut e, "SELECT ('\"7\"'::jsonb)::numeric").contains("mismatch: eval: type mismatch"));
