...
fn pg_rules_lists_every_form() {
    ...
    e.execute("CREATE RULE r_block AS ON DELETE TO rt DO INSTEAD NOTHING")
        .unwrap();
    e.execute("CREATE RULE r_cond AS ON UPDATE TO rt WHERE NEW.v < 0 DO INSTEAD NOTHING")
        .unwrap();
    e.execute(
        "CREATE RULE r_also AS ON INSERT TO rt DO ALSO INSERT INTO raud VALUES (NEW.id, NEW.v)",
    )
    .unwrap();
    let rows = texts(
        &mut e,
...
fn or_replace_swaps_rule_in_place() {
    ...
    e.execute("CREATE RULE r AS ON DELETE TO t DO INSTEAD NOTHING")
        .unwrap();
    ...
    let rows = texts(
        &mut e,
        "SELECT rulename, definition FROM pg_rules ORDER BY rulename",
    );
...
fn duplicate_without_or_replace_errors() {
    ...
    assert!(
        m.contains("rule \"r\" for relation \"t\" already exists"),
        "{m}"
    );
}
