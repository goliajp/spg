...
fn string_to_table_from() {
    ...
    assert_eq!(
        rows(&mut e, "SELECT * FROM string_to_table('a|b|c', '|') g"),
        ["a", "b", "c"]
    );
    assert_eq!(
        rows(
            &mut e,
            "SELECT * FROM regexp_split_to_table('a1b2c', '[0-9]') g"
        ),
        ["a", "b", "c"]
    );
    assert_eq!(
        rows(&mut e, "SELECT * FROM string_to_table('x,y,z,w', ',') g").len(),
        4
    );
}
...
fn string_to_table_projection_and_lateral() {
    ...
    assert_eq!(
        rows(&mut e, "SELECT string_to_table('a|b|c', '|')"),
        ["a", "b", "c"]
    );
    assert_eq!(
        rows(&mut e, "SELECT regexp_split_to_table('a1b2c', '[0-9]')"),
        ["a", "b", "c"]
    );
    ...
    assert_eq!(
        n(
            &mut e,
            "SELECT count(*) FROM t, LATERAL string_to_table(s, '|') g"
        ),
        3
    );
    assert_eq!(
        n(
            &mut e,
            "SELECT count(*) FROM t, string_to_table(t.s, '|') g"
        ),
        3
    );
}
