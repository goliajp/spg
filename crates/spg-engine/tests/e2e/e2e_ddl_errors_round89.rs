...
fn a_unknown_column_type() {
    ...
    assert!(
        err(&mut e, "CREATE TABLE bad (x notarealtype)")
...
fn b_duplicate_column_in_insert_target() {
    ...
    assert!(
        err(&mut e, "INSERT INTO t(a, a) VALUES (1, 2)")
...
fn c_add_notnull_column_no_default_on_nonempty_table() {
    ...
    assert!(
        err(&mut e, "ALTER TABLE t ADD COLUMN req int NOT NULL")
    ...
    e2.execute("ALTER TABLE u ADD COLUMN req int NOT NULL")
        .unwrap();
}
...
fn d_drop_missing_index() {
    let mut e = Engine::new();
    assert!(err(&mut e, "DROP INDEX nosuchindex").contains("index \"nosuchindex\" does not exist"));
}
...
fn e_drop_missing_view_has_no_corrupt_prefix() {
    ...
    assert!(
        msg.contains("view \"nosuchview\" does not exist"),
        "got {msg}"
    );
