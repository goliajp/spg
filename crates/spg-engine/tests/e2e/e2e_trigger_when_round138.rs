...
fn when_filters_insert_update_delete() {
    ...
    e.execute(
        "CREATE TRIGGER ti AFTER INSERT ON tt FOR EACH ROW WHEN (NEW.v > 10) EXECUTE FUNCTION lf()",
    )
    .unwrap();
    ...
    e.execute(
        "CREATE TRIGGER td BEFORE DELETE ON tt FOR EACH ROW WHEN (OLD.v < 0) EXECUTE FUNCTION lf()",
    )
    .unwrap();

    e.execute("INSERT INTO tt VALUES(1,5),(2,20),(3,50)")
        .unwrap(); // fires for 20,50
...
fn instead_of_with_when_rejected() {
    ...
    assert!(
        m.contains("INSTEAD OF triggers cannot have WHEN conditions"),
        "{m}"
    );
}
