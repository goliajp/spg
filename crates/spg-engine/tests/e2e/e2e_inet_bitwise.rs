...
fn inet_bitwise_ops() {
    ...
    assert!(b(
        &mut e,
        "SELECT ('192.168.1.5'::inet & '255.255.255.0'::inet) = '192.168.1.0'::inet"
    ));
    assert!(b(
        &mut e,
        "SELECT ('10.0.0.0/8'::inet | '0.0.0.255/24'::inet) = '10.0.0.255/24'::inet"
    ));
    assert!(b(
        &mut e,
        "SELECT (~ '255.0.0.0/8'::inet) = '0.255.255.255/8'::inet"
    ));
    // IPv6 AND.
    assert!(b(
        &mut e,
        "SELECT ('2001:db8::ff'::inet & '::f0'::inet) = '::f0'::inet"
    ));
    // Mixing families errors.
    assert!(
        e.execute("SELECT '10.1.2.3'::inet & '2001:db8::'::inet")
            .is_err()
    );
