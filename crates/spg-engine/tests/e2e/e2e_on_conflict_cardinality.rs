...
fn do_update_cannot_affect_row_twice() {
    ...
    e.execute("CREATE TABLE up (k INT PRIMARY KEY, c INT)").unwrap();
    e.execute("INSERT INTO up VALUES (1, 1)").unwrap();
    // Duplicate key already in the table → cardinality violation, nothing applied.
    assert!(e
        .execute("INSERT INTO up VALUES (1,1),(1,1) ON CONFLICT (k) DO UPDATE SET c = up.c + 1")
        .is_err());
    ...
    e.execute("CREATE TABLE af (k INT PRIMARY KEY, c INT)").unwrap();
    assert!(e
        .execute("INSERT INTO af VALUES (2,1),(2,2) ON CONFLICT (k) DO UPDATE SET c = EXCLUDED.c")
        .is_err());
}
...
fn do_nothing_and_distinct_keys_still_work() {
    ...
    e.execute("INSERT INTO af VALUES (3,1),(3,2) ON CONFLICT (k) DO NOTHING").unwrap();
    assert_eq!(scalar(&mut e, "SELECT c FROM af WHERE k=3"), Value::Int(1));
    // Distinct keys with DO UPDATE are fine.
    e.execute("INSERT INTO af VALUES (10,1),(11,2) ON CONFLICT (k) DO UPDATE SET c = EXCLUDED.c").unwrap();
