...
fn win1252_convert_to() {
    ...
    assert_eq!(
        val(&mut e, "SELECT convert_to('A', 'WIN1252')"),
        spg_storage::Value::Bytes(vec![0x41].into())
    );
    // Euro sign maps to 0x80 (WIN1252-specific, no LATIN1 equivalent).
    assert_eq!(
        val(&mut e, "SELECT convert_to(E'\\u20AC', 'WIN1252')"),
        spg_storage::Value::Bytes(vec![0x80].into())
    );
    // Alias.
    assert_eq!(
        val(&mut e, "SELECT convert_to('A', 'CP1252')"),
        spg_storage::Value::Bytes(vec![0x41].into())
    );
}
...
fn win1252_convert_from() {
    let mut e = Engine::new();
    assert_eq!(
        val(&mut e, r"SELECT convert_from('\x80'::bytea, 'WIN1252')"),
        spg_storage::Value::text("€")
    );
    // 0x41='A', 0xe9='é' (identity in the 0xA0–0xFF range, like LATIN1).
    assert_eq!(
        val(&mut e, r"SELECT convert_from('\x41e9'::bytea, 'WIN1252')"),
        spg_storage::Value::text("Aé")
    );
}
...
fn win1252_undefined_byte_errors() {
    ...
    assert!(
        e.execute(r"SELECT convert_from('\x81'::bytea, 'WIN1252')")
            .is_err()
    );
}
