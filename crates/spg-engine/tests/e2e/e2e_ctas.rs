...
fn ctas_materialises_query_result() {
    ...
    e.execute("INSERT INTO src VALUES (1, 10), (2, 20)")
        .unwrap();
    ...
    assert_eq!(
        got[0],
        vec![spg_storage::Value::Int(1), spg_storage::Value::Int(20)]
    );
    assert_eq!(
        got[1],
        vec![spg_storage::Value::Int(2), spg_storage::Value::Int(40)]
    );
}
...
fn ctas_result_is_a_plain_table() {
    ...
    assert_eq!(
        rows(&mut e, "SELECT count(*) FROM dst")[0][0],
