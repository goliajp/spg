...
fn round778_using_and_natural_over_values_answer_as_pg() {
    ...
    assert_eq!(
        grid(&mut e, "SELECT * FROM (VALUES (1,'a')) x(id,v) JOIN (VALUES (1,'b')) y(id,w) USING (id)"),
        "id|v|w;1|a|b"
    );
    assert_eq!(
        grid(
    ...
    assert_eq!(
        grid(&mut e, "SELECT count(*) FROM (VALUES (1),(2)) a(x) NATURAL JOIN (VALUES (10),(20)) b(y)"),
        "count;4"
    );
    assert_eq!(
        grid(&mut e, "SELECT * FROM (VALUES (1,'p')) a(k,s) NATURAL JOIN (VALUES (1,'q')) b(k,t)"),
        "k|s|t;1|p|q"
    );
    // Derived-select source resolves through its projection names.
    assert_eq!(
        grid(&mut e, "SELECT id FROM (SELECT 1 AS id, 'z' AS z) d JOIN (VALUES (1,'b')) y(id,w) USING (id)"),
        "id;1"
    );
}
