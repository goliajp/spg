...
fn lateral_generate_series_bare_outer_column() {
    ...
    assert_eq!(
        count(
            &mut e,
            "SELECT count(*) FROM t, LATERAL generate_series(1, n) g"
        ),
        5
    );
    assert_eq!(
        count(
            &mut e,
            "SELECT count(*) FROM t, LATERAL generate_series(1, t.n) g"
        ),
        5
    );
    ...
    assert_eq!(
        count(
            &mut e,
            "SELECT count(*) FROM t, LATERAL generate_series(1, 3) g"
        ),
        6
    );
    ...
    assert_eq!(
        count(
            &mut e,
            "SELECT count(*) FROM generate_series(1,3) g1, LATERAL generate_series(1, g1) g2"
        ),
        6
    );
}
...
fn lateral_unnest_bare_outer_column() {
    ...
    e.execute("INSERT INTO t VALUES (1, ARRAY[10,20]),(2, ARRAY[30])")
        .unwrap();
    assert_eq!(
        count(&mut e, "SELECT count(*) FROM t, LATERAL unnest(arr) u"),
        3
    );
}
