...
fn single_field_qualifiers() {
    ...
    assert_eq!(text(&mut e, "SELECT (interval '30' minute)::text"), "00:30:00");
    assert_eq!(text(&mut e, "SELECT (interval '100' second)::text"), "00:01:40");
    assert_eq!(text(&mut e, "SELECT (interval '-2' year)::text"), "-2 years");
}
...
fn single_field_truncates_to_precision_except_second() {
    ...
    assert_eq!(text(&mut e, "SELECT (interval '1.5' hour)::text"), "01:00:00");
    assert_eq!(text(&mut e, "SELECT (interval '2.5' year)::text"), "2 years");
    assert_eq!(text(&mut e, "SELECT (interval '1.5' minute)::text"), "00:01:00");
    // …but SECOND (the finest unit) keeps the fractional part.
    assert_eq!(text(&mut e, "SELECT (interval '90.5' second)::text"), "00:01:30.5");
}

#[test]
fn range_qualifiers() {
    let mut e = Engine::new();
    assert_eq!(text(&mut e, "SELECT (interval '1-6' year to month)::text"), "1 year 6 mons");
    assert_eq!(
    ...
    assert_eq!(text(&mut e, "SELECT (interval '2:03' hour to minute)::text"), "02:03:00");
}
...
fn units_bearing_literal_with_qualifier_falls_back() {
    ...
    let mut e = Engine::new();
    assert_eq!(text(&mut e, "SELECT (interval '2 days' day)::text"), "2 days");
