...
/// v7.37.42 (A3 Step 2) — same accounting as `approx_row_bytes` but
/// takes a borrowed `&[Value]` slice. The SCALARSQ streaming executor
/// keeps a reusable `Vec<Value>` scratch buffer and charges the budget
/// per row without ever wrapping it in a `Row`, so this lets it skip
/// the Row alloc on the hot path.
pub(crate) fn approx_values_bytes(values: &[Value]) -> usize {
