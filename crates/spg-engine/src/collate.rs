...
/// The MySQL charset → default-collation table now lives in `spg-sql`,
/// because the PARSER needs the same data: `_utf8mb4'x'` is an
/// introducer only when the word after `_` is a charset, and that is a
/// parsing decision. One table, asked by both — a second copy is how
/// two surfaces come to disagree.
pub(crate) use spg_sql::charset::MYSQL_CHARSET_DEFAULT_COLLATION;
...
pub(crate) fn charset_default_collation(charset: &str) -> Option<&'static str> {
    spg_sql::charset::charset_default_collation(charset)
}

pub(crate) const MYSQL_DEFAULT_CONNECTION_COLLATION: &str = "utf8mb4_0900_ai_ci";
