...
pub struct ColumnSchema {
    ...
    ///
    /// 9.0.0 — was `identity_always: bool`, which could not tell
    /// `GENERATED BY DEFAULT AS IDENTITY` from a plain `serial`: both
    /// read `false`, so `information_schema.columns` said `is_identity
    /// NO` and `pg_attribute.attidentity` an empty string for a column
    /// PostgreSQL 18.6 reports as `YES` / `BY DEFAULT` / `d`.
    ///
    /// Both flavours also set `auto_increment`; `None` with
    /// `auto_increment` is `serial`. The ALWAYS half persists from
    /// FILE_VERSION 100 (8.0.3) and the BY DEFAULT half from 102
    /// (9.0.0); an older image reads back `None`, which is what it
    /// meant before the distinction existed.
    pub identity: Option<spg_sql::ast::IdentityKind>,
...
impl Catalog {
    ...
    pub(crate) fn serialize_at(&self, version: u8) -> Vec<u8> {
        ...
        for t in &self.tables {
            ...
            if version >= 100 {
                let of_kind = |want: spg_sql::ast::IdentityKind| -> Vec<usize> {
                    t.schema
                        .columns
                        .iter()
                        .enumerate()
                        .filter_map(|(i, c)| (c.identity == Some(want)).then_some(i))
                        .collect()
                };
                let mut write_list = |list: Vec<usize>, out: &mut Vec<u8>| {
                    write_u16(out, u16::try_from(list.len()).expect("≤ 65k columns/table"));
                    for idx in list {
                        write_u16(out, u16::try_from(idx).expect("≤ 65k columns/table"));
                    }
                };
                write_list(of_kind(spg_sql::ast::IdentityKind::Always), &mut out);
                // 9.0.0 — the BY DEFAULT half, in its own appendix so an
                // image written by 8.0.x still reads: there the flavour
                // did not exist and every identity column was recorded
                // as ALWAYS or not at all.
                if version >= 102 {
                    write_list(of_kind(spg_sql::ast::IdentityKind::ByDefault), &mut out);
                }
            }
        }
