...
| 7.39 | WAL compression: LZSS encoder + decoder | Hand-rolled Storer-Szymanski 1982 sliding-window LZ in `spg_crypto::lzss` (no_std, no deps). 4 KiB window + 18-byte max match. Compress ~2× on text + ~1.5× binary | ✅ | v6.6.0, `spg_crypto::lzss` (13) |
| 7.40 | WAL compression: WAL v3 compressed-record format | New v3 type tag 0x03 wraps `[u8 algo][compressed bytes]`. Threshold-gated (SPG_COMPRESSION_MIN_BYTES, default 256). SPG_WAL_COMPRESSION env (lzss/none, default lzss). Legacy v3 type=0x01 records still replay through the upgraded binary | ✅ | v6.6.1 + v6.6.3, `e2e_wal_compression.rs` (2/2) — measured ~53% WAL bytes reduction |
| 7.41 | WAL compression: cold-tier segment v2 envelope | New segment magic SPGSEG\x02\x00 wraps the v1 layout in an LZSS envelope. OwnedSegment::from_bytes transparently unwraps v2; SegmentReader::open rejects v2 (borrowed-slice can't allocate). SPG_SEGMENT_COMPRESSION env (lzss/none, default lzss). Legacy v1 segments still load unchanged | ✅ | v6.6.2, `spg_storage::segment` (+4) |
| 7.42 | WAL compression: ratio metrics + torn-write resilience | /metrics exposes spg_wal_bytes_{uncompressed_in,compressed_out}_total + spg_segment_bytes_{uncompressed_in,compressed_out}_total counters. Per-record compression means torn writes damage only their own record; replay drops trailing partial record and recovers surviving prefix | ✅ | v6.6.3 + v6.6.4, `e2e_compression_metrics.rs` (2/2) + `chaos_wal_compression_torn_write.rs` (1/1) |
| 7.43 | Cold tier: per-table `cold_rows` precise count | `Table::cold_row_count` cached; populated by ANALYZE walking each BTree index's `RowLocator::Cold` entries. Surfaced via `spg_statistic.cold_row_count` (new BIGINT column) + `spg_stat_segment.table_name` (new TEXT column). Resolves v6.2.7 carve-out | ✅ | v6.7.0, `e2e_cold_rows_per_table.rs` (3/3) |
| 7.44 | Cold tier: BRIN-style segment-level sidecar | `CREATE INDEX … USING BRIN (col)` parser + `IndexKind::Brin` variant + segment v2 envelope BRIN sidecar emit/parse round-trip. 21-byte entries (page_index u32 + sentinel u8 + min/max u64). Planner page-skipping is OOS for v6.7 (STABILITY carve-out) | ✅ | v6.7.1, `e2e_brin.rs` (3/3) |
| 7.45 | Cold tier: per-table hot/cold byte budget | `ALTER TABLE t SET hot_tier_bytes = X` parser + `TableSchema.hot_tier_bytes: Option<u64>` field + freezer integration (per-table override takes precedence over global `SPG_HOT_TIER_BYTES`) + catalog snapshot v11 → v12. spg_table_ddl re-emission is OOS for v6.7 (STABILITY carve-out) | ✅ | v6.7.2, `e2e_per_table_budget.rs` (5/5) |
| 7.46 | Cold tier: segment compaction (LSM merge + GC) | `COMPACT COLD SEGMENTS` admin-only SQL merges segments smaller than `SPG_COMPACTION_TARGET_SEGMENT_BYTES` (default 4 MiB) into one segment. DELETE'd-but-frozen rows pruned during merge. `cold_segments: Vec<Option<Arc<…>>>` keeps segment_id stable across tombstoning. Chaos test verifies manifest swap atomicity under kill -9 | ✅ | v6.7.3, `e2e_compaction.rs` (4/4) + `chaos_compact_atomic.rs` (2/2) |
| 7.47 | Cold tier: parallel freezer worker pool | `SPG_FREEZER_WORKERS` env (default `max(1, num_cpus()-2)`, cap 16). `Catalog::{prepare_freeze_slice, commit_freeze_slices}` split the freeze into N parallel prepare slices + 1 coordinator commit (encode_segment serial per §6). Prepare phase 2.21× speedup at 4 workers vs 1 on 14-core M-series host | ✅ | v6.7.4, `e2e_parallel_freezer.rs` (1/1) + `perf_parallel_freezer.rs` (#[ignore]) |
| 7.48 | Cold tier: segment forwarding replication | v2 frame type `FRAME_TYPE_SEGMENT_FILE_CHUNK = 0x03`, payload `[u32 segment_id][u32 chunk_seq][u32 chunk_total][u32 chunk_bytes ≤ 16 MiB][bytes]`. Master ships cold segments to V2 followers before WAL tail begins; follower commits via `Engine::receive_cold_segment`. Segment-level resume via on-disk file existence (chunk-level OOS for v6.7) | ✅ | v6.7.5, `e2e_segment_forward.rs` (3/3) |
| 7.49 | Cold tier: boot-time prefetch worker pool | `SPG_PREFETCH_WORKERS` env (default `max(1, num_cpus()-2)`, cap 16). Boot path parallelises cold-segment file reads; Linux uses `posix_fadvise(WILLNEED)` to overlap kernel read-ahead with the next worker's I/O. Metric `spg_cold_prefetch_hits_total`. 2.48× speedup at 4 workers vs 1 over 32 × 8 MiB segments | ✅ | v6.7.6, `e2e_prefetch.rs` (1/1) + `perf_prefetch.rs` (#[ignore]) |
| 7.50 | Cold tier: 1B-row pipeline stress test | `pipeline_sanity_50k_rows` (default sweep) exercises INSERT → freezer → compaction → CHECKPOINT → cold restart in ~10 s wall-time; ~18 ms cold-start. `cold_start_under_120s` (#[ignore]) is the L2 ship gate at operator-tunable scale via `SPG_PERF_1B_ROW_BUDGET` (default 1_000_000). Real-1B-row run is operator-driven (~30 min, ~TB disk) | ✅ | v6.7.7, `perf_1b_rows.rs` (1/1 + 1 #[ignore]) |
| 7.51 | Index breadth: INCLUDE columns | `CREATE INDEX … (key) INCLUDE (col, …)` parser + `Index.included_columns: Vec<usize>` + catalog snapshot FILE_VERSION 11 → 12. INCLUDE rejected on HNSW + BRIN. In-BTree-leaf payload storage + planner `index only scan` are STABILITY carve-outs | ✅ | v6.8.0, `e2e_include.rs` (7/7) |
| 7.52 | Index breadth: Partial index | `CREATE INDEX … WHERE <expr>` parser + `Index.partial_predicate: Option<String>` (canonical Display). WHERE rejected on HNSW + BRIN. Maintenance is over-maintenance (every row enters partial idx, correctness preserved); planner-side "use partial when query WHERE implies predicate" pass is STABILITY carve-out | ✅ | v6.8.1, `e2e_partial_index.rs` (7/7) |
| 7.53 | Index breadth: Expression index | `CREATE INDEX … (lower(col))` parser supports any Pratt-parseable expression as the key. `Index.expression: Option<String>` (canonical Display) persists across snapshot round-trip; primary column derived from `extract_first_column`. Expression-on-HNSW + BRIN rejected. Runtime expr-key derivation = STABILITY carve-out | ✅ | v6.8.2, `e2e_expression_index.rs` (7/7) |
| 7.54 | Index breadth: Index advisor | `EXPLAIN (SUGGEST) <SELECT>` walks WHERE / JOIN-ON column refs, resolves owners, dedupes by (table, column), emits `SUGGEST: CREATE INDEX <auto-name> ON <table> (<col>)` per missing index. Pure-syntax heuristic — cost-based ranking is future v6.x | ✅ | v6.8.3, `e2e_index_advisor.rs` (7/7) |
| 7.55 | Concurrency: bench-driven evaluation | `tests/perf_concurrency.rs` (#[ignore]) drives 8/16/32 native-wire clients at the engine under SELECT-only + mixed (75/25) workloads. 14-core M-series measurement: SELECT 122k → 143k ops/s (1.17× scale 8→32), mixed 2.5k → 9.3k ops/s (3.7× scale). p99 ≤ 16ms at 32 clients in mixed mode. Choice A (parallel prepare + OCC retry) carved out to v7.x — numbers above the typical OLTP target operating point | ✅ | v6.9.0 + v6.9.1, `perf_concurrency.rs` (1 #[ignore]) |
| 7.56 | SPG-unique: WAL-as-SQL pub/sub | `SPG_PUBSUB_TARGET=log` fans every successfully-committed SQL out as a NATS-framed `PUB <subject> <bytes>\r\n<payload>\r\n` line. `SPG_PUBSUB_SUBJECT` (default `spg.wal.sql`) routes. Real-broker TCP target carved out to STABILITY | ✅ | v6.10.0, `pubsub::tests` (3/3) |
| 7.57 | SPG-unique: per-query CPU/wall budget | `SPG_MAX_QUERY_NS` (ns-resolution) sits alongside `SPG_QUERY_TIMEOUT_MS` (ms-resolution). Watchdog picks tighter deadline; sleep slice tightened to `max(total/50, 100µs)` so sub-ms budgets fire on time | ✅ | v6.10.1, `e2e_query_ns_budget.rs` (3/3) |
| 7.58 | SPG-unique: cold-tier time travel | `SELECT … FROM <tbl> AS OF SEGMENT '<id>'` scans only the named cold segment. Scope: projection + WHERE + LIMIT. JOINs / aggregates / ORDER BY surface a STABILITY carve-out error. `AS OF TIMESTAMP` parked (needs per-segment timestamp metadata) | ✅ | v6.10.2, `e2e_as_of_segment.rs` (6/6) |
| 7.59 | SPG-unique: embedded mode | New `spg-embedded` crate. `Database::open_in_memory`, `execute(sql)`, `query(sql)`, `snapshot()`, `restore(bytes)`. `FromSpgRow` trait sketch reserves the shape for the future `#[derive(SpgRow)]` macro | ✅ | v6.10.3, `spg-embedded::tests` (4/4) + doc-test (1/1) |
| 7.60 | SPG-unique: WAL replay sandbox | `spg-server --replay-only` boot path: restore catalog snapshot, replay WAL, exit 0. No listener, no audit chain, no replication. Sandboxed by design — poisonous SQL surfaces an error before any live deployment sees it | ✅ | v6.10.4, `e2e_replay_only.rs` (2/2) |
