# spg → sentori — 7.38.24, and the bill 7.38.22 sent you

**Image:** `goliakk/spg:7.38.24`
**Manifest digest:** `sha256:5672641c7fb4aff51463106f701083b0fa90c39fc7c61b219eb1ff293e960c40`
**Battery:** the release train's own — a dogfood replay, then
`gate.sh all`: lint, unit, e2e, gates, biz, dogfood, and a
release-blocking perf comparison against PostgreSQL 18.4 with both legs
under `C` (64 cells, **no losses**, one cell's verdict withdrawn because
the binary separated from itself and that cell is not counted either
way). Then drop-in acceptance against the pushed image: **71/71**.

It took three attempts, and the two failures are worth stating rather
than smoothing over. The first stopped at our own lint gate — over a
lint in a test we had written that morning, which our per-commit gate
could not see because it was not linting test code at all; that is fixed
and is why the version number skips. The second stopped because the perf
comparison had no live PostgreSQL to compare against, and our gate
treats "nothing was compared" as a failure on a release run rather than
a skip. We did not wave either one through.
...
You will notice the version goes from 7.38.22 to 7.38.24. 7.38.23 was
tagged and never released: its release train stopped at our own lint
gate, the triage ended in a fix to a gate, and we do not rewrite a tag
that has been pushed. Nothing was published under it.
...
Same query, same fixture, on the binary this release ships:
    ...
    declaring a collation that orders this data exactly as bytes do
      7.38.22          2.91x
      7.38.24          0.99x   (spread 0.96-1.08)
