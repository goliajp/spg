...
| 每接进点 1 testcase 验证 attach + wakeup 可见 | ⚠️ 5/9 | `aggregate_spill_trigger` + 本 turn 加的 4(`planner_first_row_fetch` / `index_build_post_seal` / `tx_commit_walgroup_leader_switch` / `wal_group_commit_leader_chosen`)— 全 PASS。tests at `crates/spg-engine/src/tests.rs::inject_*` | **剩 4 接进点的 testcase**:`checkpoint_cow_swap_pre/post` 需触发 checkpoint event(Database::Drop 间接路径);`cold_tier_wakeup_resume` 需 `Database::open_path`(integration 级);`spg_sqlx_inline_budget_cancel` 需 spg-sqlx async runtime。这 4 个不属于 lib unit test 范围,defer 入 integration-tier tests |
