...
///
/// The fixture (`xtests/compat-datadirs/v7.38.1/`) was captured by
/// the v7.38.1 tag's own binary: 500 rows across nine types, two
...
pub fn ironrules_full(root: &Path, runid: &str) -> Result<String, String> {
    ...
    let fixture = root.join("xtests/compat-datadirs/v7.38.1");
    ...
    for line in expected.lines() {
        ...
        if let Some(e) = r.error {
            return Err(format!("v7.38.1 fixture: {sql}: {e}"));
        }
        let got = r
            .rows
            .first()
        ...
        if got != want {
            return Err(format!(
                "v7.38.1 fixture: {key}: want {want}, got {got} — the previous \
    ...
    roster.reap_all();
    let _ = std::fs::remove_dir_all(&tmp);
    Ok(format!("{smoke}; v7.38.1 dir direct-open verified"))
}
