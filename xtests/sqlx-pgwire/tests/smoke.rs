...
async fn jsonb_round_trip_via_serde_json() {
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_jsonb").execute(&pool).await.unwrap();
...
async fn timestamptz_decodes_into_datetime_utc() {
    ...
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_ts").execute(&pool).await.unwrap();
...
async fn returning_id_from_insert() {
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_ret").execute(&pool).await.unwrap();
...
async fn on_conflict_do_nothing_dedup() {
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_uniq").execute(&pool).await.unwrap();
...
async fn on_conflict_do_update_excluded() {
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_acc").execute(&pool).await.unwrap();
    ...
    sqlx::query("INSERT INTO sqlx_acc VALUES (1, 'old')")
        .execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO sqlx_acc VALUES (1, 'new') \
...
async fn on_conflict_composite_target_do_update() {
    let pool = pool().await;
    sqlx::query("DROP TABLE IF EXISTS sqlx_cal").execute(&pool).await.unwrap();
    sqlx::query(
    ...
    sqlx::query("INSERT INTO sqlx_cal VALUES (1, 100, 'v1')")
        .execute(&pool).await.unwrap();
    sqlx::query("INSERT INTO sqlx_cal VALUES (1, 100, 'v2') \
