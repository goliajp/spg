...
integer` — at 183 sites, plus the `requires` and `expects` spellings
elsewhere. PostgreSQL 18.6 answers `function lower(integer) does not
exist` for every one, and names a bare string literal `unknown`, not
`text`: `strpos(1,'a')` is `function strpos(integer, unknown) does not
exist` there, and `nosuchfn('a','b')` is `(unknown, unknown)`.

The rewrite sits at the call boundary, where the call's own expressions
are in hand — the same place two earlier rounds already rename operands
PostgreSQL names differently. It fires only when every argument can be
typed; where the expression cannot be typed and the call has one
argument, the evaluator's own message already named that type and it is
used. `bool_and` / `bool_or` / `unnest` raise outside that boundary and
were changed at the five sites that write them.

Six pins had recorded SPG's former wording as the expectation — one of
them asserting `got integer` where its own comment said "PG says the
function does not exist for integer, SPG that it needs an interval".
They assert PostgreSQL's sentence now, measured.

NOT closed, measured and recorded: a call whose ONE argument is a bare
string literal. PostgreSQL commits the literal to the only candidate's
type and reports the input function's error (`abs('x')` is `invalid
input syntax for type double precision: "x"`), which needs a
per-function candidate table. Those keep the evaluator's own sentence
rather than a newly wrong one.

### Fixed — `GENERATED BY DEFAULT AS IDENTITY` was not an identity column

The column schema recorded one bool, `identity_always`, which answered
"is this the ALWAYS one" and could not tell `BY DEFAULT` from a plain
`serial`: both read `false`. So `information_schema.columns` reported
`is_identity NO` and `pg_attribute.attidentity` an empty string —
PostgreSQL's answer for "not an identity column at all" — for a column
that is one. It is a three-state now, and a `serial` still reads `NO`.

FILE_VERSION 102 carries the BY DEFAULT half in its own appendix beside
the 8.0.3 ALWAYS one, so an image written by 8.0.x still reads: there
the flavour did not exist and every identity column was recorded as
ALWAYS or not at all.

The three ALTER forms went with it. Measured against PostgreSQL 18.6:

* `ALTER COLUMN c ADD GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY`
  lowered to the auto-increment marker alone, losing which flavour it
  was, so the column read `is_identity NO` afterwards. It also skipped
  both of PostgreSQL's refusals: a NULLABLE column (`must be declared
  NOT NULL before identity can be added`) and a second ADD (`is already
  an identity column`).
* `ALTER COLUMN c SET GENERATED { ALWAYS | BY DEFAULT }` was swallowed
  by the SET tail's catch-all.
* `ALTER COLUMN c DROP IDENTITY IF EXISTS` on a plain column said
  nothing, where PostgreSQL emits `NOTICE: column "id" of relation "t"
  is not an identity column, skipping` — the whole point of that
  spelling being that the operator wanted to know either way.
...
dump failed to restore into PostgreSQL 18.6 on `role "postgres" does not
exist` and on `CREATE EXTENSION` for five extensions the schema never
created. Run against their own seventeen migrations it was worse:
`pg_dump` stopped (`function pg_get_function_arguments(bigint) does not
exist`), and once it ran, the dump restored nowhere (`SUPPORT 0`). This is
the path an operator leaving SPG takes.

The criterion is now the dump's TEXT: the same schema built on SPG and on
PostgreSQL, dumped by the same `pg_dump`, must produce the same dump, and
SPG's dump must restore into PostgreSQL with `ON_ERROR_STOP` and into SPG
as a fixed point. That holds for the release gate's fixture and for
sentori's seventeen migrations. What was wrong, each measured against
18.6:

- **Owners.** Every owner column was the constant 10, named `postgres`;
  `pg_tables` said `admin`; views and types recorded no owner at all. Roles
  are numbered once now (the bootstrap superuser at 10, PostgreSQL's
  predefined roles at their own oids), views, types, materialized views
  and CTAS tables record their creator, and `ALTER TABLE … OWNER TO`
  records the new one. `public` belongs to `pg_database_owner` and keeps
  its `standard public schema` comment.
- **Extensions.** `pg_extension` listed what the build provides. It lists
  `plpgsql` and what `CREATE EXTENSION` installed, in the schema named;
  `DROP EXTENSION [IF EXISTS]` removes, and a duplicate or missing name is
  refused with PostgreSQL's words (42710 / 42704). Extension comments are
  published.
- **Names under an empty search path.** `pg_dump` reads every definition
  with `search_path = ''`. `current_schemas` ignored it, and view bodies,
  column types, foreign keys, defaults, index expressions and trigger
  functions came back unqualified — restoring stopped on the first of
  them. They are qualified when the path leaves `public` out, as
  PostgreSQL does.
- **Serial columns.** Their sequence existed only once something named
  it, so the dump had a default naming a sequence it never created. It is
  listed from the start, owned by its column (`pg_depend` `a`), with its
  `nextval(…::regclass)` default in `pg_attrdef`.
- **Constraints.** `conindid` was 0, so every primary key and UNIQUE
  constraint dumped as a bare `CREATE UNIQUE INDEX`. A table rebuild
  (DROP COLUMN, VACUUM) also dropped the flags tying an index to its
  constraint, which published SPG's internal probe indexes as user
  indexes. Domain constraints had no rows, so domains dumped without their
  CHECK. CHECK constraints and index predicates print in PostgreSQL's
  catalog form (`= ANY (ARRAY['a'::text, …])`, `WHERE active`).
- **Functions and triggers.** `pg_get_function_arguments`,
  `…_identity_arguments`, `…_result` and `…_sqlbody` did not exist;
  `prosupport` read `0` for `-`; a PL/pgSQL function's `prolang` matched no
  `pg_language` row; `pg_get_triggerdef` answered NULL, so every trigger
  dumped as a bare `;`. `COMMENT ON INDEX` was a no-op, function and type
  comments matched no object, and `COMMENT ON VIEW` was refused.
- **Details that change the restored schema.** An index's operator class
  (`jsonb_path_ops`) was not recorded; an enum column reported a collation
  (`COLLATE` on an enum is refused by PostgreSQL); `attlen`/`attstorage`
  came from a seven-type table (`SET STORAGE EXTENDED` on every uuid
  column); `typcollation` was 0 everywhere; `relam` was 0; a partition's
  date bound printed as timestamptz; a materialized view's column carried
  a NOT NULL; a view over a join lost its closing parenthesis, because
  `pg_dump` strips the definition's last character expecting `;`.
- **Restoring into SPG.** `ALTER SEQUENCE … OWNER TO`, `ALTER DOMAIN …
  OWNER TO`, `::public.mood` and `DEFAULT 'ok'::public.mood` were refused.

Found on the way and fixed: an expression index over a user-defined
function made every INSERT into its table fail (`function f(text) does
not exist`) — the key was computed with no catalog to find the function.
`GENERATED ALWAYS AS IDENTITY` was not persisted, so after a restart the
column accepted explicit values.

The on-disk format moves to FILE_VERSION 100 (owners of views and types,
installed extensions, index operator classes, identity-always columns);
an 8.0.2 binary refuses an 8.0.3 data directory by its version, as every
format step before this one.

Named and not changed: SPG stores a PL/pgSQL body re-rendered, not as
written, so `prosrc` differs in layout; a view over a join is not laid
out the way PostgreSQL's deparse lays it out (the text is valid and
restores); SERIAL and `GENERATED BY DEFAULT AS IDENTITY` share one
auto-increment flag and dump as SERIAL; `ALTER TABLE … ADD GENERATED
ALWAYS AS IDENTITY` restores as BY DEFAULT; `ALTER SEQUENCE / VIEW / TYPE
/ FUNCTION … OWNER TO` check the role and record nothing; a `DEFAULT
nextval('s')` numbers the column max+1 and not from the sequence
(RD-12, `docs/RECORDED_DELTAS.md`); `CREATE SCHEMA` creates nothing — the
last four need AST or storage fields that are breaking changes.

The release gate's `pgdump-roundtrip` step moves from the full tier to
the prerelease tier. It used to discard the PostgreSQL restore's errors
and compare row counts, which is why a restore failing on every `OWNER
TO` passed; it now stops on the first error, compares the dump text with
PostgreSQL's, checks the fixed point, and its fixture carries the shapes
sentori's schema broke on.
