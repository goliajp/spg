...
| **v6.4.4** | `INSERT ... ON CONFLICT DO SELECT [FOR UPDATE]`. Already supported in v5.x as an INSERT extension; v6.4.4 adds the `DO SELECT` arm that returns the matched-or-inserted row (PG 19 surface). | `tests/e2e_insert_on_conflict_do_select::matches_existing_row` + `…::inserts_new_row` + `…::for_update_locks_row_under_concurrent_tx` | v6.3.6 |
